    ...
    return (
      <div className='restaurant_select'>
        <div className='restaurant_select_top'>
          <div onClick={this.displayList} className='restaurant_select_top_header font-affect-outline'>{this.state.title ? this.state.title : 'Выбери ресторан'}</div>
          ...
          { this.state.display ? <div className='restaurant_select_bottom'>
            <ul>
              {
                restaurants.map((restaurant) => <li onClick={() => this.getTitle(restaurant)} key={restaurant.id}>{restaurant.title}</li>)
              }
            </ul>
          </div> : null}
        </div>
